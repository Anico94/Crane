...
from reaction_gui import Reactions
...
#reactions = ['Fx','Fy','Fz','Mx','My','Mz'] these will be selected from the gui
reactions = selected_reactions(gui_outputs[0])
...
filename = f'{gui_outputs[2]}/{gui_outputs[3]}.csv'                      
...
# print ('{}xlsx'.format(filename[:-3]))

read_csv_file = pd.read_csv(filename)
excelwriter = pd.ExcelWriter('{}xlsx'.format(filename[:-3]))
read_csv_file.to_excel(excelwriter)
excelwriter.save()